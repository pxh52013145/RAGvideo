...
request.interceptors.response.use(
  (response: AxiosResponse<IResponse>) => {
    ...
    if (res.code === 0) {
      ...
      if (!silent) toast.error(res.msg || '操作失败，请稍后再试');
      return Promise.reject(res); // 拒绝Promise，让业务代码可以捕获并处理
    }
  },
  (error) => {
    const silent = Boolean(
      (error?.config as RequestConfig | undefined)?.silent ||
        (error?.response?.config as RequestConfig | undefined)?.silent
    )
    // 网络/服务器错误
